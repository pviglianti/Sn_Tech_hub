...
## Artifact to Classify (#17 of 956)

ID: 203980
Name: Mark Invoice Attached /TIme on WO
Table: sys_script
...
(function executeRule(current, previous /*null when async*/ ) {
    // runs when incidental type of vendor aka invoice is inserted, parent WO is T&M and has invoice is false
    var wmOrder = current.parent_task.parent;
    //check true on has Invoice and add current date and time to invoice attached on
    var woGr = new GlideRecord('wm_order');
    if (woGr.get(wmOrder)) {
        woGr.u_has_invoice = "true";

        var invTime = new GlideDateTime();
        woGr.u_invoice_attached_on = invTime;
