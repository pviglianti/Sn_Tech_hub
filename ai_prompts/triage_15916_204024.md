...
## Artifact to Classify (#42 of 956)

ID: 204024
Name: Auto populate Sev 3 members - CI
Table: sys_script
...
(function executeRule(current, previous /*null when async*/) {
	...
	var woParticipants = [];
	var incParticipants = [];
	var woAssignees = current.getValue('additional_assignee_list') || '';
	var incID = current.getValue('initiated_from');

	var sev3GroupID = '1d3570e11b705550d0e3ec20604bcb12'; //Sev 3 Affected Item Leadership Notification group
	
	
	var incGR = new GlideRecord('incident');
	incGR.get(incID);
	
	var incShortDesc = incGR.getValue('short_description').toLowerCase();
	
	
if (incShortDesc.includes('sev 3')) {
		var incAssignees = incGR.getValue('additional_assignee_list') || '';
		
		var grMember = new GlideRecord('sys_user_grmember');
		grMember.addQuery('group', sev3GroupID);
		grMember.query();
		
		while (grMember.next()){
	
			var user = grMember.getValue('user');
			
			if (!woAssignees.includes(user) || gs.nil(woAssignees)) {
				woParticipants.push(user);
			}
			if (!incAssignees.includes(user) || gs.nil(incAssignees)) {
				incParticipants.push(user);
			}
			}
	
			incGR.additional_assignee_list = woAssignees + ',' + woParticipants.join();
			incGR.update();
			
		
			current.additional_assignee_list =  incAssignees + ',' + incParticipants.join();
			current.update();
