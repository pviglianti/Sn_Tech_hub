...
## Artifact to Classify (#22 of 956)

ID: 203985
Name: Update WOT Assigned to/Assignment Group
Table: sys_script
...
(function executeRule(current, previous /*null when async*/ ) {
    ...
    var util = new FSMAssignmentUtilsBVP();
    var isDynamic = util.isDynamicAssignmentGroup(assignmentGroup);

    // if assigned to changes but we're dynamic and the group is the same, we are done
    if (isDynamic && (assignmentGroup == previous.assignment_group)) {
        return;
    }

    // ...otherwise, if we're dynamic, clear out assigned to.  for everything else, map from INC -> WOT
    var assignedTo = isDynamic ? 'NULL' : current.assigned_to + '';

    var gr = new GlideRecord('wm_task');
    if (gr.get(current.u_work_order_task)) {
        gs.eventQueue('bvp.fsm.assign_wot', gr, assignmentGroup, assignedTo);
    } else {
		throw 'Business Rule "Update WOT Assigned to/Assignment Group" has no associated wm_task';
	}
