...
## Artifact to Classify (#46 of 956)

ID: 204028
Name: Populate Assignment from INC
Table: sys_script
...
(function executeRule(current /*, previous */) {
  // Get initiating Incident safely
  var inc = current.initiated_from ? current.initiated_from.getRefRecord() : null;
  if (!inc || !inc.isValidRecord()) return;

  // Pass sys_ids (critical)
  var group = inc.getValue('assignment_group') || '';
  var assn  = inc.getValue('assigned_to')      || '';
  if (!group) return;

  // Try to find the first ACTIVE WOT created synchronously with the WO
  var wot = new GlideRecord('wm_task');
  wot.addQuery('work_order', current.getUniqueValue());
  wot.addActiveQuery();
  wot.orderBy('sys_created_on');
  wot.setLimit(1);
  wot.query();
  if (!wot.next()) return; // if WOT not there yet, the Incident rule will pick it up later

  // Single writer path: custom event handled by Script Action
  gs.eventQueue('bvp.fsm.assign_wot', wot, group, assn);
})(current);
```
