...
## Artifact to Classify (#2 of 956)

ID: 203964
Name: Check for Open Work Orders on Close
Table: sys_script
...
function onBefore(current, previous) {
	var iid = current.sys_id.toString();
	if(!iid){
		return; // invalid id, end here
	}
	
	//get existing related work order query
	var woQuery = 'stateNOT IN16,18,20,22^initiated_from=';
	woQuery += iid;
	//woQuery += '^ORparent=';
	//woQuery += iid;
	
	var wo = new GlideRecord('wm_order');
	wo.addEncodedQuery(woQuery);
	wo.setLimit(1);
	wo.query();
	if(wo.hasNext()) {
		var ewoMessage = gs.getMessage('Related Work Order(s) must be completed or confirmed before closing this incident.');
		gs.addErrorMessage(ewoMessage);
