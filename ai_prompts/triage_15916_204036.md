...
## Artifact to Classify (#54 of 956)

ID: 204036
Name: Category Mapping on Display
Table: sys_script
...
(function executeRule(current, previous /*null when async*/ ) {
    ...
    var location = current.task.getDisplayValue();

    var a = current.task.getRefRecord();


    if (location.indexOf("REQ") > -1) {
        current.setValue('category', 'Support');
     //   current.update();
    }

    if (location.indexOf("INC") > -1) {
        current.setValue('category', 'Support');
       // current.update();
    }

    if (location.indexOf("PRB") > -1) {
        current.setValue('category', 'Support');
       // current.update();
    }

    if (location.indexOf("CHG") > -1) {
        current.setValue('category', 'Support');
      //  current.update();
    }

        if (location.indexOf("TASK") > -1) {
            current.setValue('category', 'Support');
         //   current.update();
        }

    if (location.indexOf("RITM") > -1) {
        current.setValue('category', 'Support');
       // current.update();
    }


    var b = a.top_task.getRefRecord();

    if (location.indexOf("PRJTASK") > -1) {
     //   gs.addInfoMessage("PRJTASK");
        if (b.investment_type == 'functional' || b.investment_type == 'enterprise') {
            current.setValue('category', 'work');
          //  current.update();
        }
    } else {
        if (location.indexOf("PRJ") > -1) {
         //   gs.addInfoMessage("PRJ");
            if (a.investment_type == 'functional' || a.investment_type == 'enterprise') {
                current.setValue('category', 'work');
            //    current.update();
            }
        }
    ...
    if (location.indexOf("PRJTASK") > -1) {
       // gs.addInfoMessage("PRJTASK");
        if (b.investment_type == 'small_work') {
            current.setValue('category', 'Small Work');
            //current.update();
        }
    } else {
        if (location.indexOf("PRJ") > -1) {
          //  gs.addInfoMessage("PRJ");
            if (a.investment_type == 'small_work') {
                current.setValue('category', 'Small Work');
              // current
... (truncated)
```
Artifact collection: time_card
