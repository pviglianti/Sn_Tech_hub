...
## Artifact to Classify (#38 of 956)

ID: 204020
Name: PCGTryResolveParentIncident
Table: sys_script
Target Table: wm_order
Class: sys_script
...
function onAfter(current, previous) {
		tryResolveParentIncident(current);
}

function tryResolveParentIncident(current) {
	if(canResolveParentIncident(current)){
		resolveIncident(current);
	}
}

function canResolveParentIncident(current) {
	return !canNOTResolveParentIncident(current);
}

function canNOTResolveParentIncident(current) {
	var workOrdersWithDifferentAssignmentGroupThanParentIncident = getWorkOrdersWithDifferentAssignmentGroupThanParentIncidentQuery(current);
	var thereExistsWorkOrdersWithDiffAssignmentGroup = workOrdersWithDifferentAssignmentGroupThanParentIncident.next();
	
	var workOrdersThatAreNotClosedCanceledOrConfirmed = getWorkOrdersThatAreNotClosedCanceledOrConfirmed(current);
	var thereExistsWorkOrdersThatAreNotClosedCanceledOrConfirmed = workOrdersThatAreNotClosedCanceledOrConfirmed.next();
	
	var workOrdersInTheConfirmedStateButNotInTheReadyToCloseSubstate = getWorkOrdersInTheConfirmedStateButNotInTheReadyToCloseSubstate(current);
	var thereExistsWorkOrdersInTheConfirmedStateButNotInTheReadyToCloseSubstate = workOrdersInTheConfirmedStateButNotInTheReadyToCloseSubstate.next();
	
	return thereExistsWorkOrdersWithDiffAssignmentGroup || thereExistsWorkOrdersThatAreNotClosedCanceledOrConfirmed || thereExistsWorkOrdersInTheConfirmedStateButNotInTheReadyToCloseSubstate;
}

function getWorkOrdersWithDifferentAssignmentGroupThanParentIncidentQuery(current) {
	var assignmentGroupId = current.initiated_from.assignment_group;
	var incidentId = current.initiated_from;
	var workOrders = new GlideRecord("wm_order");
	//workOrders.addEncodedQuery("initiated_from=" + incidentId + "^assignment_group!=" + assignmentGroupId);
	workOrders.addEncodedQuery("initiated_from=" + incidentId + "^assignment_group.typeNOT LIKE4214509f4fab8e00334cd0af0310c759");
							   
	workOrders.query();
	//var msg=workOrders.getRowCount();


//gs.addErrorMessage('<script>var t=setTimeout(function(){var test = confirm("' + msg + '");if(!test){return false;}},500);</script>' + msg)
... (truncated)
```
Artifact collection: wm_order
