...
## Artifact to Classify (#30 of 956)

ID: 203999
Name: Resolve Associated Incident
Table: sys_script
...
function onAfter(current, previous) {
	//if initiated from is an open incident, resolve it
	var iniFrom = current.initiated_from;
	var myLinkedWorkOrder = current.sys_id.toString();
	var woAssignedGroup = current.assignment_group;
	var incAssignedGroup = current.initiated_from.assignment_group;
	
	var ifRec = new GlideRecord('task');
	ifRec.get('sys_id',iniFrom);
		
	if(!ifRec){
		return;
	}
	
	if(ifRec.sys_class_name != 'incident'){
		//not an incident, end here
		return;
	}
	
	if(woAssignedGroup != incAssignedGroup){
		//incident does not have the same group as work order, end here
		return;
	}
	
	var incState = ifRec.state;
	if(incState == 6 || incState == 7 || incState == 8){
		//incident is already inactive, end here
		return;
	}
	
	if(hasOtherOpenWorkOrders(ifRec.sys_id.toString(),current.sys_id.toString(),woAssignedGroup)){
		//incident has other open work orders associated, end here
		return;
	}
	...
	var incNumber = ifRec.number;
	ifRec.update();
		gs.addInfoMessage('Resovled associated incident ' + incNumber);
}

function hasOtherOpenWorkOrders(iid,woid,wag){
	//check for other open work orders associated with this incident
	//if found and assignment group is different from wo group, return true
	var owAnswer = false;
	if(!iid){
		return owAnswer;
	}
	...
	//build query for other work orders
	var stateQuery = 'stateNOT IN18,20,22^initiated_from=';
	stateQuery += iid;
	stateQuery += '^sys_id!=';
	stateQuery += woid;
	stateQuery += '^NQstate=18^u_substate!=90^initiated_from=';
	stateQuery += iid;
	stateQuery += '^sys_id!=';
	stateQuery += woid;
	
	var wogr = new GlideRecord('wm_order');
	wogr.addEncodedQuery(stateQuery);
	wogr.query();
	
	while(wogr.next()){
		//check if the open work order is assigned to the same group as this one
		if(wogr.assignment_group != wag){
			//match found and assigned to another group, return true
			owAnswer = true;
		}
	}
