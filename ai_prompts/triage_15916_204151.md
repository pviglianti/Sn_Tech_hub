...
## Artifact to Classify (#72 of 956)

ID: 204151
Name: Sync District fields
Table: sys_script
...
(function executeRule(current, previous /*null when async*/) {
	...
	//Get all Locations in this district and Sync fields unless field is listed in override
	var district = current.sys_id;
	var locationGR = new GlideRecord('cmn_location');
	locationGR.addQuery('u_district_number', district);
	locationGR.query();
	
	
	while (locationGR.next()) {
		if (locationGR.u_override_district_sync == false) {

			locationGR.u_cust_experience_specialist = current.u_cust_experience_specialist;
			locationGR.u_district_ap_manager = current.u_district_ap_manager;
			//locationGR.u_pharmacy_supervisor = current.u_pharmacy_supervisor;
			locationGR.u_bakery_specialist = current.u_bakery_specialist;
			locationGR.u_hr_talent_manager = current.u_hr_talent_manager;
			locationGR.u_deli_specialist = current.u_deli_specialist;
			locationGR.u_floral_specialist = current.u_floral_specialist;
			locationGR.u_meat_specialist = current.u_meat_specialist;
			locationGR.u_produce_specialist = current.u_produce_specialist;
			locationGR.u_seafood_specialist = current.u_seafood_specialist;
			
			//DSD fields
			locationGR.u_center_store_specialist = current.u_center_store_specialist;
			locationGR.u_regional_operations_specialist = current.u_regional_operations_specialist;
			locationGR.u_district_manager = current.u_district_manager;
			
			
			locationGR.update();
		} else {
	
			var overRideFields = locationGR.u_district_fields_to_override;
			...
			if (overRideFields.indexOf('Bakery Specialist') == -1) {
				locationGR.u_bakery_specialist = curr
... (truncated)
```
