...
## Artifact to Classify (#68 of 956)

ID: 204113
Name: Update Related Update Sets on RLS Change
Table: sys_script
...
(function executeRule(current, previous /*null when async*/) {
	var fetr = current.sys_id;
	var updateset = new GlideRecord('u_update_set_management');
	updateset.addQuery('u_related_feature', fetr.toString());
	updateset.query();
	
	
	while (updateset.next())
		{
			updateset.u_release = current.parent;
			updateset.update();
			gs.log('update set sys: ' + updateset.sys_id.toString());
		}
