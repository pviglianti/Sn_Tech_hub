...
## Artifact to Classify (#32 of 956)

ID: 204002
Name: Auto-Close Parent Incident on Closure
Table: sys_script
...
function onAfter(current, previous) {
	//automatically close related incident if no other work orders associated
	var woid = current.sys_id.toString();
	var incid = current.initiated_from.toString();
	
	var wogr = new GlideRecord('wm_order');
	wogr.addQuery('initiated_from',incid);
	wogr.addQuery('sys_id','!=',woid);
	...
	incgr.get('sys_id',incid);
	
	if(incgr){
		if(incgr.state != 7 && incgr.state != 8){
			incgr.state = 7;
			incgr.update();
		}
	}
