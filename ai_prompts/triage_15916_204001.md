...
## Artifact to Classify (#31 of 956)

ID: 204001
Name: PCG_CloseRelatedTaskOnComplete
Table: sys_script
...
function onAfter(current, previous) {
	var taskType = current.u_related_task.sys_class_name;
	if(!gs.nil(taskType)){
		var taskGR = new GlideRecord(taskType);
		taskGR.get('' + current.u_related_task);
		
		if(taskType == 'incident'){
			taskGR.state = '7';
			taskGR.close_code = 'Solved (Permanently)';
			taskGR.close_notes = 'Feature has been implemented';
		}
		else if(taskType == 'u_ad_hoc_request'){
			taskGR.state = '3';
			taskGR.close_notes = 'Feature has been implemented';
		}
