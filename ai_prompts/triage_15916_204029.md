...
## Artifact to Classify (#47 of 956)

ID: 204029
Name: Sync State with Parent
Table: sys_script
...
(function executeRule(current, previous) {
	var EXCLUDED_GRP = '97dddf194f2e420017e6d49f0310c739'; // IT Service Desk
	gs.info("TR WO Sync State 500: "+current.initiated_from.assignment_group.sys_id);
	gs.info("TR WO Sync State 501: "+current.initiated_from.assignment_group.name);

	// ---- Only run when the WO transitions to closed/inactive ----
	// Prefer active flag (robust across state values). Requires BR runs on update.
	if (!previous) return;
	if (previous.active == current.active) return;     // no active flag change
	if (current.active == true) return;                // not a transition to inactive/closed

	// Load Incident by initiated_from
	var inc = new GlideRecord('incident');
	var incSysId = current.getValue('initiated_from');
	if (!incSysId || !inc.get(incSysId)) {
		return;
	}

	// ---- Only let the *current* WO control the Incident ----
	// If the Incident has moved on and now points to a *different* WO, don't touch it.
	var incCurrentWO = inc.getValue('u_work_order'); // reference to wm_order
	if (!incCurrentWO || incCurrentWO != current.getUniqueValue()) {
		return;
	}

	// ---- Skip if Incident is with Service Desk ----
	if (inc.getValue('assignment_group') == EXCLUDED_GRP) {
		return;
	}

	// ---- Your original mapping/notes stay the same ----
	var woState = current.getValue('state');
	inc.setValue('state', woState == '7' ? '8' : '7'); // your original 7<->8 mapping
	inc.work_notes.setJournalEntry('Automatic closure from Work Order closure.');
	inc.close_notes.setJournalEntry('Automatic closure from Work Order closure.');
	inc.close_code.setValue('Solved (Permanently)');
	inc.update();
})(current, previous);
```
