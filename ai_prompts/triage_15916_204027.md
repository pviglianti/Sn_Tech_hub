...
## Artifact to Classify (#45 of 956)

ID: 204027
Name: WM Task Auto-Gen Limit
Table: sys_script
...
(function executeRule(current, previous /*null when async*/) {
    ...
    if (!woGr.get(current.work_order)) {
        return;
    }

	var fsmUtils = new FSMUtilsBVP();
	var canCreate = fsmUtils.wotIncTaskLimit(current.work_order.toString());
	
	if (!canCreate) {
		gs.addErrorMessage('Cannot create work order task: This work order already has an active task.');
