...
## Artifact to Classify (#25 of 956)

ID: 203988
Name: Generate IWTM Workday Code Lookup Values
Table: sys_script
Target Table: sm_incidentals
Class: sys_script
...
```
/*
 * Business Rule: Populate Workday Accounting Codes
 * Table: sm_incidentals
 * When: before insert, before update
 * Order: 110 (just after the rule that sets u_accounting_code_legacy)
 * Condition: !gs.nil(current.u_accounting_code_legacy) && current.u_accounting_code_legacy.changes()
 */
(function executeRule(current /*, previous */) {

	// Safety check – nothing to do if the legacy code is empty
	var code = (current.u_accounting_code_legacy + '').trim();
	if (!code) {
		current.u_wd_acct_code_iwtm1 = '';
		current.u_wd_acct_code_iwtm2 = '';
		return;
	}

	// Split on the *last* dash so we're bullet-proof if the first half also has dashes
	var lastDash = code.lastIndexOf('-');
	if (lastDash === -1) {
		// No dash at all – treat the whole thing as invalid / blank-out children
		current.u_wd_acct_code_iwtm1 = '';
		current.u_wd_acct_code_iwtm2 = '';
		return;
	}

	// Part before the final dash = IWTM1 (strip trailing dash by definition)
	var iwtm1 = code.substring(0, lastDash);
	// Part after the final dash = IWTM2
	var iwtm2 = code.substring(lastDash + 1);

	/* ---- light validation (optional) ----
	   • IWTM2 is always six digits
	   • IWTM1 must have at least one character
	   If it doesn't pass, we just leave the fields blank; the on-change client script
	   you already have will stop the user from moving to "Incurred".
	*/
	if (!/^\d{6}$/.test(iwtm2) || !iwtm1) {
		current.u_wd_acct_code_iwtm1 = '';
		current.u_wd_acct_code_iwtm2 = '';
		return;
	}

	current.u_wd_acct_code_iwtm1 = iwtm1;
	current.u_wd_acct_code_iwtm2 = iwtm2;

})(current, previous);
```
Artifact collection: sm_incidentals
