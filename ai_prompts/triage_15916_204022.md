...
## Artifact to Classify (#40 of 956)

ID: 204022
Name: Mark Parent as Associated
Table: sys_script
...
function onAfter(current, previous) {
	//mark the 'associated work order' reporting flag if initiated from an incident
	var ifgr = current.u_initiated_from.getRefRecord();
	if(ifgr){
		if(ifgr.sys_class_name == 'incident' || ifgr.sys_class_name == 'u_ad_hoc_request' ){
			ifgr.u_associated_work_order = true;
				ifgr.update();
			}
	}
}
