...
## Artifact to Classify (#73 of 956)

ID: 204152
Name: Sync Region fields
Table: sys_script
...
(function executeRule(current, previous /*null when async*/) {
	...
	var region = current.sys_id;
	var locationGR = new GlideRecord('cmn_location');
	locationGR.addQuery('u_region_number', region);
	locationGR.query();
	
	while (locationGR.next()) {
		if (locationGR.u_override_region_sync == false) {
			locationGR.u_regional_vp = current.u_regional_vp;
			locationGR.u_regional_admin_assistant = current.u_regional_admin_assistant;
			locationGR.u_regional_ap_manager = current.u_regional_ap_manager;
			locationGR.u_regional_hr_manager = current.u_regional_hr_manager;
			locationGR.u_regional_ce_manager = current.u_regional_ce_manager;
			locationGR.u_regional_food_safety_specialist = current.u_regional_food_safety_specialist;
			//locationGR.u_regional_pharmacy_manager = current.u_regional_pharmacy_manager;
			locationGR.u_ap_field_specialist = current.u_ap_field_specialist;

			locationGR.update();
		} else {
			var overRideFields = locationGR.u_fields_to_override;
			...
			//if (overRideFields.indexOf('Regional 
... (truncated)
```
