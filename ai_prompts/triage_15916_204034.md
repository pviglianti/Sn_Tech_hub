...
## Artifact to Classify (#52 of 956)

ID: 204034
Name: PCG_rollUpWorkAndTravelTime
Table: sys_script
...
function onAfter(current, previous) {
	...
	tw.addQuery("task", current.task.sys_id + '');
	...
	if (tw.next()) {
		timeWorked = tw.getAggregate("SUM", "time_worked");
		gs.log("Jim time worked = " + timeTraveled);
	}
	
	var tt = new GlideAggregate("task_time_worked");
	tt.addQuery("task", current.task.sys_id + '');
	tt.addQuery("u_type", "Travel");
	tt.addAggregate("SUM", "time_worked");
	tt.groupBy("task");
	tt.query();
	if (tt.next()) {
		timeTraveled = tt.getAggregate("SUM", "time_worked");
		gs.log("Jim time traveled = " + timeTraveled);
	}
	
	var total = new GlideAggregate("task_time_worked");
	total.addQuery("task", current.task.sys_id + '');
	total.addAggregate("SUM", "time_worked");
	total.groupBy("task");
	total.query();
	if (total.next()) {
		totTime = total.getAggregate("SUM", "time_worked");
		gs.log("Jim total time worked = " + totTime);
	}
	
	var inc = current.task.getRefRecord();
		  
	inc.setValue("calendar_duration", timeWorked);
	inc.setValue("u_travel_duration", timeTraveled);
	inc.setValue("u_combined_time", totTime);
