...
## Artifact to Classify (#4 of 956)

ID: 203967
Name: Create Work for record producers
Table: sys_script
...
(function executeRule(current, previous /*null when async*/ ) {
    ...
    //checks work order table for assignment records and sets assigned to user
	var assignGroup = current.assignment_group;
	var shortDes = current.short_description;
// 	if (shortDes == 'Weights & Measures Failure - Register Scale'){
// 		assignGroup = '85d203d34f1df600e81ca35e0210c7a8';
// 	}
	

    var assignTo = new GlideRecord('u_work_order_assignment');
    assignTo.addQuery('u_affected_item', current.u_business_service);
    assignTo.addQuery('u_configuration_item', current.cmdb_ci);
    assignTo.addQuery('u_assignment_group', assignGroup);
    assignTo.addQuery('u_location', current.location);
    assignTo.query();
    if (assignTo.next()) {
        var assignUser = '';
        if (assignTo.u_assigned_to != '') {
            assignUser = assignTo.u_assigned_to;
        } else if (assignTo.u_assigned_to_secondary != '') {
            assignUser = assignTo.u_assigned_to_secondary;
        } else if (assignTo.u_assigned_to_tertiary != '') {
            assignUser = assignTo.u_assigned_to_tertiary;
        } else if (assignTo.u_off_hour_assigned_to != '') {
            assignUser = assignTo.u_off_hour_assigned_to;
        } else if (assignTo.u_off_hour_assigned_to_secondary != '') {
            assignUser = assignTo.u_off_hour_assigned_to_secondary;
        } else if (assignTo.u_off_hour_assigned_to_tertiary != '') {
            assignUser = assignTo.u_off_hour_assigned_to_tertiary;
        }
    }
    current.assigned_to = assignUser;

    //set accounting code from location, department, and business service
    var locationCode = current.location.u_location_code;
    var storeNum = current.location.u_store;
    var didValue = current.u_department.id;
    var department = didValue.slice(-2);
    var accountCode = current.u_business_service.u_accounting_code;
    var setCode = true;
 
    var fullAccountingCode;
	fullAccountingCode = locationCode + "-" + storeNum + "-" + department + "-" + accountC
... (truncated)
```
