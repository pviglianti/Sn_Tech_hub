...
## Artifact to Classify (#41 of 956)

ID: 204023
Name: LIFT - Intake Automation
Table: sys_script
Target Table: ticket
Class: sys_script
...
function automateIntake() {
	var helper = new USDIntakeHelper();
	var targetRecord = helper.automateIntake(current);
	
	if(targetRecord != '') {
		current.active = false;
		current.state = 3;
		//appendIntakeWorkNotesToTargetRecordDescription(targetRecord);
		tryBuildWorkOrderWithTargetRecord(targetRecord);
	}
}

//function appendIntakeWorkNotesToTargetRecordDescription(targetRecord) {
//	targetRecord.description = targetRecord.description + "\n\n" + "Intake Work Notes:\n" + current.work_notes.getJournalEntry(-1);
//	gs.addInfoMessage("" + targetRecord.description);
//	targetRecord.update();
//}

function tryBuildWorkOrderWithTargetRecord(targetRecord) {
	if(shouldBuildWorkOrder()) {
		var wmOrderRec = getWorkOrderQueryFromIncident(targetRecord); 
		if(!wmOrderRec.next()){
...
function getWorkOrderQueryFromIncident(incident) {
	var workOrder = new GlideRecord("wm_order");
	workOrder.addQuery('initiated_from', incident.sys_id);
	workOrder.query();
	return workOrder;
}
...
function updateIncident(incident) {
	...
	incident.u_associated_work_order = true;
	inc
... (truncated)
```
Artifact collection: ticket
