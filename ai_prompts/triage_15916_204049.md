...
## Artifact to Classify (#63 of 956)

ID: 204049
Name: Generate accounting code update
Table: sys_script
...
function onBefore(current, previous) {
    ...
    gs.log("This business rule is called", "ap-debug-06");
    ...
    var accountCode = '';
    var setCode = true;
    var displayIncomplete = true;
    var diSetting = gs.getProperty('weis.wm_order.accounting_code.allow_incomplete');

    if (diSetting == false || diSetting == 'false') {
        displayIncomplete = false;
    }

    if (!JSUtil.nil(current.location)) {

        var llcValue = current.location.u_location_code;
        if (!llcValue || llcValue == '') {
            llcValue = '';
            setCode = false;
        }
        locationCode = llcValue;

        var lsnValue = current.location.u_store;
        if (!lsnValue || lsnValue == '') {
            lsnValue = '';
            setCode = false;
        }
        storeNum = lsnValue;
    ...
    if (!JSUtil.nil(current.u_department)) {

        var didValue = current.u_department.id;
        if (!didValue || didValue == '') {
            didValue = '';
            setCode = false;
        }
        department = didValue.slice(-2);
    } else {
        setCode = false;
    }

    if (!JSUtil.nil(current.u_business_service.u_accounting_code)) {

        var bscValue = current.u_business_service.u_accounting_code;
        if (!bscValue || bscValue == '') {
            bscValue = '';
            setCode = false;
        }
        accountCode = bscValue;
    } else {
        setCode = false;
    }
...
//***************************************************************************
//***************************************************************************
//Added by George 1/23/2024
//used to force the following CI's to account code 514143 
//so Jason Kaufman will get them
//if ((current.cmdb_ci =='Major Spill - Contractor Required') || (current.cmdb_ci == 'Spill Kit Pick-up')){
if ((current.
... (truncated)
```
