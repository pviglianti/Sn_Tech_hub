...
## Artifact to Classify (#70 of 956)

ID: 204126
Name: Creating user from task record
Table: sys_script
...
(function executeRule(current, previous /*null when async*/ ) {
    ...
    var createUser = new GlideRecord('sys_user');
    createUser.initialize();
	createUser.user_name = current.variables.ad_user_name;
    createUser.u_preferred_first_name = current.variables.requested_for.toString().split(' ')[0];
	createUser.first_name = current.variables.requested_for.toString().split(' ')[0];
    createUser.last_name = current.variables.requested_for.toString().split(' ')[1];

    if (current.variables.organization == '3rd PARTY VENDOR/OTHER') {
        createUser.u_ps_set_id.setDisplayValue('VENDOR');
		createUser.u_date_to_review_vendor = current.variables.date_renewal_or_revoked;
    }
	
	    if (current.variables.will_a_weis_email_be_needed == 'No') {
     		createUser.email = current.variables.provide_a_3rd_party_vendor_email;
		    }
    createUser.manager = current.variables.who_owns_the_relationship_with_this_3rd_party_vendor;
    createUser.insert();
	

})(current, previous);
```
Artifact collection: sc_task
