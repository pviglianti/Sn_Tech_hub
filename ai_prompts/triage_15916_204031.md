...
## Artifact to Classify (#49 of 956)

ID: 204031
Name: Map WO Fields to WOT - update
Table: sys_script
...
(function executeRule(current, previous) {
    ...
    var util = new FSMUtilsBVP();

    // map fields to WOT
    var wotGr = new GlideRecord('wm_task');
    wotGr.addQuery('work_order', current.sys_id);
    wotGr.addActiveQuery();
    wotGr.query();

    while (wotGr.next()) {
        util.mapFields(current, wotGr);
		wotGr.u_requested_by = current.getValue('caller'); //Caller on Task is Caller
		wotGr.opened_for = current.getValue('opened_for'); //on behalf of to on behalf of
		if(current.getValue('location') !== previous.getValue('location')) {
			wotGr.location = current.location;
		}
        wotGr.update();
    }
