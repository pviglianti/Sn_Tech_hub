...
## Artifact to Classify (#36 of 956)

ID: 204017
Name: On-Call Trigger Events
Table: sys_script
Target Table: task
Class: sys_script
...
function onAfter(current, previous) {
	//find on-call trigger rules for this task and group
	var tn = current.sys_class_name;
	var tid = current.sys_id.toString();
	var tgroup = current.assignment_group.sys_id.toString();
	...
	//query the trigger rule table for manual override rules
	var trgr = new GlideRecord('trigger_rule');
	trgr.addQuery('table',tn);
	trgr.addQuery('u_override_assignment',true);
	trgr.addQuery('group',tgroup); //matches the assignment group
	trgr.addQuery('order','!=',''); //ignore records without an order value
	trgr.orderBy('order');
	trgr.query();
	...
	while(trgr.next()){
		var triggerID = trgr.sys_id.toString();
		var cond = trgr.condition;
		if(!cond){
			//condition is empty, consider as match
			gs.eventQueue('trigger_rule.process',current,'trigger_rule',triggerID);
			hasRota = true;
			break;
		}
		else if(GlideFilter.checkRecord(current, cond)){
			gs.eventQueue('trigger_rule.process',current,'trigger_rule',triggerID);
			hasRota = true;
			break;
		}
		//no conditions matched, continue to next result
	}
	
	//results complete, check for a match and run code for unmatched group
	if(!hasRota){
		//queue non-rota group assignment event for specific table
		if(tn == 'incident'){
			gs.eventQueue('incident.non_rota.group.assigned',current,tgroup,tn);
		}
	}
}
```
