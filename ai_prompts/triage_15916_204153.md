...
## Artifact to Classify (#74 of 956)

ID: 204153
Name: Sync Region fields on Region change
Table: sys_script
...
(function executeRule(current, previous /*null when async*/) {
	...
	//Get all Locations in this region and Sync fields unless field is listed in override
	var region = current.u_region_number;
	var locationGR = new GlideRecord('cmn_location');
	locationGR.get(region);

		if (current.u_override_region_sync == false) {
			current.u_regional_vp = locationGR.u_regional_vp;
			current.u_regional_admin_assistant = locationGR.u_regional_admin_assistant;
			current.u_regional_ap_manager = locationGR.u_regional_ap_manager;
			current.u_regional_hr_manager = locationGR.u_regional_hr_manager;
			current.u_regional_ce_manager = locationGR.u_regional_ce_manager;
			current.u_regional_food_safety_specialist = locationGR.u_regional_food_safety_specialist;
			//current.u_regional_pharmacy_manager = locationGR.u_regional_pharmacy_manager;
			current.u_ap_field_specialist = locationGR.u_ap_field_specialist;

	
		} else {
			var overRideFields = current.u_fields_to_override;
			
			if (overRideFields.indexOf('Regional VP') == -1) {
				current.u_regional_vp = locationGR.u_regional_vp;
			}
			
			if (overRideFields.indexOf('Regional Admin Assistant') == -1) {
				current.u_regional_admin_assistant = locationGR.u_regional_admin_assistant;
			}
			
			if (overRideFields.indexOf('Regional AP Manager') == -1) {
				current.u_regional_ap_manager = locationGR.u_regional_ap_manager;
				
			}
			
			if (overRideFields.indexOf('Regional CE Manager') == -1) {
				current.u_regional_ce_manager = locationGR.u_regional_ce_manager;
				
			}
			
			if (overRideFields.indexOf('Regional Food Safety Specialist') == -1) {
				current.u_regional_food_safety_specialist = locationGR.u_regional_food_safety_specialist;
			}
			
			
			if (overRideFields.indexOf('Regional HR Manager') == -1) {
				current.u_regional_hr_manager = locationGR.u_regional_hr_manager;
				
			}
			
			//if (overRideFields.indexOf('Regional Pharmacy Manager') == -1) {
			//	current.u_regional_pharmacy_manager = locationGR.u_regional_
... (truncated)
```
