...
## Artifact to Classify (#48 of 956)

ID: 204030
Name: Emer Chg - Add Incident to Inc Pend Chg
Table: sys_script
...
Script/Code:
```
// BR Table: change_request
// When: after (insert + update)
// Condition (recommended): current.u_incident.changes() || (!previous && current.u_incident)

(function executeRule(current, previous) {

  // No incident on the change? nothing to do
  if (!current.u_incident)
    return;

  // Load the incident referenced by change_request.u_incident
  var inc = new GlideRecord('incident');
  if (!inc.get(current.u_incident))
    return;

  // Field on Incident to store the Change reference (OOTB is "rfc")
  var INCIDENT_CHANGE_FIELD = 'rfc'; // change this if you use a custom field

  if (!inc.isValidField(INCIDENT_CHANGE_FIELD)) {
    gs.warn('BR Change->Incident sync: Incident field not found: ' + INCIDENT_CHANGE_FIELD);
    return;
  }

  // Avoid pointless updates
  if (inc.getValue(INCIDENT_CHANGE_FIELD) == current.getUniqueValue())
    return;

  // Keep it quiet (optional but usually desired)
  inc.setWorkflow(false);

  // Set incident.rfc = this change
  inc.setValue(INCIDENT_CHANGE_FIELD, current.getUniqueValue());
  inc.update();

})(current, previous);
```
Artifact collection: change_request
