...
## Artifact to Classify (#65 of 956)

ID: 204096
Name: Restrict Manager Membership Creation
Table: sys_script
...
Script/Code:
```
//restrict non-admin users from adding group members if they are not a manager
if(verifyRights()){
	current.setAbortAction(true);
	gs.addErrorMessage("You do not have the required role to add/remove/update users to/from this group");
}

function verifyRights() {
	var stopActionAnswer = false;
	if(gs.hasRole('user_admin')){
		return stopActionAnswer;
	}
	...
	//Check if user is the manager of the selected group
	var groupRec = current.group.getRefRecord();
	if(!groupRec){
		return stopActionAnswer;
	}
	
	if(groupRec.manager != gs.getUserID()){
		stopActionAnswer = true;
	}
	
	return stopActionAnswer;
}
```
Artifact collection: sys_user_grmember
