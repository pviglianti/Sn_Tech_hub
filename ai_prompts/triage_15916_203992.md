...
## Artifact to Classify (#29 of 956)

ID: 203992
Name: Synchronize Work Notes
Table: sys_script
...
(function executeRule(current, previous /*null when async*/ ) {
    const recentWorkNote = current.work_notes.getJournalEntry(1) + '';

    const idx = recentWorkNote.indexOf('\n');
    const note = idx >= 0 ? recentWorkNote.substring(idx + 1) : '';

    const wmOrderGr = new GlideRecord('wm_order');
    wmOrderGr.addQuery('initiated_from', current.sys_id + '');
    wmOrderGr.query();
    while (wmOrderGr.next()) {
        wmOrderGr.setWorkflow(false);
        wmOrderGr.work_notes = note;
        wmOrderGr.update();
		wmOrderGr.setWorkflow(true);

        const wmTaskGr = new GlideRecord('wm_task');
        wmTaskGr.addQuery('wm_order', wmOrderGr.sys_id + '');
        wmTaskGr.query();
        while (wmTaskGr.next()) {
            wmTaskGr.setWorkflow(false);
            wmTaskGr.work_notes = note;
            wmTaskGr.update();
			wmOrderGr.setWorkflow(true);
        }
    }
