...
## Artifact to Classify (#58 of 956)

ID: 204040
Name: DNU updtVendor #on Inv if Assign Changes
Table: sys_script
Target Table: wm_task
Class: sys_script
...
(function executeRule(current /*, previous */) {
    ...
    /* -----------------------------------------------------------------
     * 1. Short-circuit if no assigned_to (rare but possible)
     * -----------------------------------------------------------------*/
    if (JSUtil.nil(current.assigned_to)) {
        return;    // nothing (or unassigned)—stop here
    }

    /* -----------------------------------------------------------------
     * 2. Get the technician's vendor number from the User record
     * -----------------------------------------------------------------*/
    var tech = current.assigned_to.getRefRecord();               // best-practice getRefRecord() :contentReference[oaicite:2]{index=2}
    if (!tech.isValidRecord() || JSUtil.nil(tech.u_vendor_number)) {
        return;    // invalid ref or vendor number blank
    }

    var vendorNum = tech.u_vendor_number.toString();

    /* -----------------------------------------------------------------
     * 3. Find all Incidentals whose parent_task == this WOT
     * -----------------------------------------------------------------*/
    var inc = new GlideRecord('sm_incidentals');
    inc.addQuery('parent_task', current.sys_id);
    inc.query();
    if (!inc.hasNext()) {
    ...
    /* -----------------------------------------------------------------
     * 4. Push the value down; setForceUpdate() guarantees DB write
     * -----------------------------------------------------------------*/
    while (inc.next()) {
        inc.u_vendor_reference  = vendorNum;              // <- replace with your target field
        inc.setWorkflow(false);                // don't re-trigger child flows
        inc.autoSysFields(false);              // leave Updated by/On alone
        inc.setForceUpdate(true);              // bypass hasChanges() check :contentReference[oaicite:3]{index=3}
        inc.update();
    }

})(current, previous);
```
Artifact collection: wm_task
