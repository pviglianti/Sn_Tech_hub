...
## Artifact to Classify (#57 of 956)

ID: 204039
Name: Sync accounting code from parent
Table: sys_script
...
(function executeRule(current /*, previous */) {
	...
	// ----------------------------------------------------------------------
	// 1. Re-calculate the legacy accounting code for THIS wm_task
	// ----------------------------------------------------------------------
	var util   = new WeisAccountingCode();   // Script-Include you already created
	var result = util.build(current);        // { code:'AAA-BB-CC-DDD', missing:[…] }

	// ----------------------------------------------------------------------
	// 2. Push that code down to every related sm_incidental
	// ----------------------------------------------------------------------
	var inc = new GlideRecord('sm_incidentals');
	inc.addQuery('parent_task', current.sys_id);
	inc.addQuery('u_override_accounting_code_value', false);
	inc.addQuery('u_workday_integration_status', '!=', 'success');

	inc.query();
	while (inc.next()) {

		inc.setValue('u_accounting_code_legacy', result.code);

		// Force the row to update even if the value hasn't "changed"
	//	inc.setWorkflow(false);        // avoid refiring child BRs
		inc.autoSysFields(false);      // keep audit log tidy
		inc.setForceUpdate(true);      // <-- bypass hasChanges() optimisation
		inc.update();

		// -- optional: comment out the log line once you're happy --
		// gs.info('ACCT-SYNC: refreshed ' + inc.number, 'ACCT-SYNC');
	}
