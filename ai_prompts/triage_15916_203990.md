...
## Artifact to Classify (#27 of 956)

ID: 203990
Name: Publish latest quote to scratchpad
Table: sys_script
...
var inclGR=new GlideRecord('sm_incidentals');
inclGR.addQuery('parent_task',current.parent_task);
inclGR.addQuery('type','quote');
inclGR.addQuery('state','quote_approved');
inclGR.setLimit(1);
inclGR.orderByDesc('u_quote_approval_date');
inclGR.query();
if(inclGR.next()){
	g_scratchpad.quote=inclGR.cost;
	g_scratchpad.quote_rec=inclGR.sys_id;
}
