...
## Artifact to Classify (#28 of 956)

ID: 203991
Name: Get Vendor ID Number/Reference
Table: sys_script
...
(function executeRule(current, previous /* null on insert */) {
    ...
    // 1. Bail out early if there is no parent_task
    if (JSUtil.nil(current.parent_task)) {
        return;
    }

    // 2. Load the Work Order Task record
    var wot = current.parent_task.getRefRecord();
    if (!wot.isValidRecord()) {
        return;                 // bad reference, nothing to do
    }

    // 3. We need the user assigned to that WOT
    if (JSUtil.nil(wot.assigned_to)) {
        return;                 // no assignee yet
    }

    var tech = wot.assigned_to.getRefRecord();
    if (!tech.isValidRecord()) {
        return;                 // shouldn't happen, but be safe
    }

    // 4. Copy the custom vendor number field
    current.u_vendor_reference = tech.u_vendor_number;   // swap names if yours differ
