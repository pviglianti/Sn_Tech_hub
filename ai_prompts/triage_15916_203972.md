...
## Artifact to Classify (#9 of 956)

ID: 203972
Name: Invoice Approval/Rejection Date/Time
Table: sys_script
...
(function executeRule(current, previous) {
    /* ---------- INVOICE path (type = vendor) ---------- */
    if (current.type == 'vendor') {
        // APPROVAL: transition to 'incurred'
        if (current.state.changesTo('incurred')) {
            current.u_inv_approval_date = gs.nowDateTime();
            current.u_approver = gs.getUserID(); // who approved it
        }

        // REJECTION: transition to 'inv_rejected'
        if (current.state.changesTo('inv_rejected')) {
            current.u_invoice_rejected_date = gs.nowDateTime();
			current.u_approver = gs.getUserID(); // who rejected it
        }
    }

    /* ---------- QUOTE path (type = quote) ------------ */
    if (current.type == 'quote') {
        // APPROVAL: transition to 'quote_approved'
        if (current.state.changesTo('quote_approved')) {
            current.u_quote_approval_date = gs.nowDateTime();
            current.u_approver = gs.getUserID(); // who approved it
        }

        // REJECTION: transition to 'quote_reject'
        if (current.state.changesTo('quote_reject')) {
            current.u_quote_reject_date = gs.nowDateTime();
			current.u_approver = gs.getUserID(); // who rejected it it
        }
    }
})(current, previous);
```
