...
## Artifact to Classify (#7 of 956)

ID: 203970
Name: Create/update expense line - Invoice WOT
Table: sys_script
...
Script/Code:
```
createExpLine(current);

function createExpLine(current) {
   var gr = new GlideRecord('fm_expense_line');
   
   if (!gr.get(current.expense_line) || current.operation() == 'insert') {
      gr.initialize();
      gr.setValue('state', 'processed');
      gr.setValue('short_description', 'Automatically generated expense line for incidentals');
	  gr.setValue('amount', current.cost);
	  gr.setValue('task', current.parent_task);
      current.expense_line = gr.insert();
   } else {
      gr.setValue('amount', current.cost);
      gr.update();
   }
