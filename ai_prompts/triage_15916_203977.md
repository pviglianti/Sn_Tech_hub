...
## Artifact to Classify (#14 of 956)

ID: 203977
Name: Prevent Approval if legacy code incorrec
Table: sys_script
...
(function executeRule(current, previous) {
    ...
    var task = current.parent_task.getRefRecord();
    if (!task.isValidRecord())
        return;      // invalid parent – another control can catch that

    var util   = new WeisAccountingCode();
    var result = util.build(task);

    if (result.missing.length > 0) {
        gs.addErrorMessage(
            'Cannot move to Incurred – missing: ' + result.missing.join(', ')
        );
        current.state = 'pending';      // revert only the state
        current.setAbortAction(false);  // allow other field updates
    }
