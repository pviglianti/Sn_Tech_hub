...
## Artifact to Classify (#26 of 956)

ID: 203989
Name: Block Creation of Inv if one exists
Table: sys_script
...
(function executeRule(current /*, previous*/) {
  ...
  // Choice VALUES (adjust if different in your instance)
  var STATE_PENDING  = 'pending';
  var STATE_INCURRED = 'incurred';
  var TYPE_VENDOR    = 'vendor';

  // Only care on Vendor records
  if (current.getValue('type') !== TYPE_VENDOR)
    return;

  var wotId = current.getValue('parent_task');
  if (!wotId) return; // nothing to enforce without a parent WOT

  // Does a Vendor incidental already exist for this WOT in pending or incurred?
  var dup = new GlideRecord('sm_incidentals');
  dup.addQuery('parent_task', wotId);
  dup.addQuery('type', TYPE_VENDOR);
  dup.addQuery('state', 'IN', STATE_PENDING + ',' + STATE_INCURRED);
  dup.setLimit(1);
  dup.query();

  if (dup.next()) {
    gs.addErrorMessage('A Vendor incidental already exists in Pending or Incurred for this Work Order Task.');
