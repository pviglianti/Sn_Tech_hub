...
## Artifact to Classify (#24 of 956)

ID: 203987
Name: Map INC fields to WO
Table: sys_script
...
(function executeRule(current, previous /*null when async*/ ) {
	gs.log('STU _ MAP INC');
    //var woGr = new WorkManagementInitiation().checkIfWorkOrderExistsAndReturn(current);
	var bvpUtil = new FSMUtilsBVP();
	var woGr = new GlideRecord('wm_order');

	if(!woGr.get(current.u_work_order)) 
		return;	
		gs.log('STU _ MAP INC ' + woGr.sys_id);

	var owningTeam = bvpUtil.getOwningTeamFromIncident(current);
	
	// if the originating incident is not longer a work order team, close out the WO
	if(!owningTeam) {
		woGr.setValue('state', '7');
		woGr.work_notes = 'Auto closing as originating Incident Assignment Group does not require a Work Order.';
		woGr.update();
		return;
	}

	// map fields/owning team to WO...
    bvpUtil.mapFields(current, woGr);
	woGr.setValue('category', owningTeam);
	
	if(current.getValue('location') !== previous.getValue('location')) {
		woGr.setValue('location', current.location);
	}
	if(current.u_list_level != previous.u_list_level)
		woGr.setValue('u_list_level',current.u_list_level);

	// map subcat from incident group
	var woSubcategory = new FSMAssignmentUtilsBVP().getSubcategory(current.getValue('assignment_group'));
	woGr.subcategory = woSubcategory;
