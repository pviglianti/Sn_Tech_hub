...
## Artifact to Classify (#15 of 956)

ID: 203978
Name: Map Workday worktags to list
Table: sys_script
...
(function executeRule(current, previous /* null when async */) {
  ...
  function repop(which) {
    // which = 1 or 2
    var codeField = (which === 1) ? 'u_wd_acct_code_iwtm1' : 'u_wd_acct_code_iwtm2';
    var listField = (which === 1) ? 'u_iwtm_1_worktags'   : 'u_iwtm_2_worktags';
    var relDot    = (which === 1)
      ? 'u_related_mapping_record_iwtm1.u_external_code'
      : 'u_related_mapping_record_iwtm2.u_external_code';

    var code = trim(current.getValue(codeField));

    // Always clear first
    current.setValue(listField, '');

    if (!code)
      return;

    var gr = new GlideRecord('u_workday_worktags');
    // Mirror the list filter behavior: dot-walk + STARTSWITH
    gr.addQuery(relDot, 'STARTSWITH', code);
    // gr.setLimit(1000); // optional safety cap
    gr.query();
    ...
    if (ids.length)
      current.setValue(listField, ids.join(','));
  }

  // Run on insert or when either code changes (keeps it efficient on updates)
  if (current.operation() == 'insert' ||
      current.u_wd_acct_code_iwtm1.changes() ||
      current.u_wd_acct_code_iwtm2.changes()) {
    repop(1);
    repop(2);
  }
