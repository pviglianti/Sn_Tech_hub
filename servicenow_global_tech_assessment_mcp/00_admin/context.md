...
- Current execution focus: post-Phase-7 hardening is in place and Phase 9/10 delivery has been implemented (pipeline prompt integration toggle, report exports, process recommendations UI, and assessment summary dashboard). Targeted Phase 9/10 regression is green (`13 passed`).
...
- **Phase 9/10 implementation complete** (Codex, 2026-03-05): wired registered MCP prompts into pipeline handlers behind `pipeline.use_registered_prompts`, added Excel/Word exports from `assessment_report` payloads, added assessment-detail Process Recommendations tab (`/api/assessments/{id}/process-recommendations/*`), and added cross-assessment summary dashboard at `/assessments/summary`.
